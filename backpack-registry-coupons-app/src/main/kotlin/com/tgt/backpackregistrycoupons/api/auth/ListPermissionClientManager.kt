...
class ListPermissionClientManager(private val permissionsClient: ListPermissionsClient) : ListPermissionManager {
    ...
    override fun authorize(userId: String, listId: UUID, requestMethod: HttpMethod): Mono<Boolean> {
        val permissionType: PermissionType = when (requestMethod) {
            ...
            else -> throw ForbiddenException(FORBIDDEN_ERROR_CODE(listOf("User is not allowed to access List $listId")))
        }
