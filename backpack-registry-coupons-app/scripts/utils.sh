...
function get_enterprise_secret_token() {
 ...
 function read_enterprise_secret_value() {
   ...
   log_msg "read_enterprise_secret_value: fetching Enterprise Secret token value for ${org}/${__env_name}/${gitrepo}/$appname"
   get_enterprise_secret_token false vault_token
   secret_value=`curl -k -s -w 'httpcode:%{http_Code}\n' ${VAULT_ADDR}/v1/secret/lists-v4/${org}/${__env_name}/${gitrepo}/$appname -H "X-Vault-Token: ${vault_token}" 2>&1`
   if [[ "$secret_value" =~ .*"httpcode:403".* ]]; then
     ...
     secret_value=`curl -k -s ${VAULT_ADDR}/v1/secret/lists-v4/${org}/${__env_name}/${gitrepo}/$appname -H "X-Vault-Token: ${vault_token}" 2>&1`
   elif [[ "$secret_value" =~ .*"httpcode:404".* ]]; then
     log_msg "read_enterprise_secret_value: No secret value found in Enterprise Secrets"
 ...
 function write_enterprise_secret_value() {
   ...
   log_msg "write_enterprise_secret_value: write Enterprise Secret token value for ${org}/${__env_name}/${gitrepo}/$appname"
   get_enterprise_secret_token false vault_token
   secret_value=`curl -k -s -w 'httpcode:%{http_Code}\n' -X POST ${VAULT_ADDR}/v1/secret/lists-v4/${org}/${__env_name}/${gitrepo}/$appname -H "X-Vault-Token: ${vault_token}" -H "Content-Type: application/json" --data '{"'$PRIVKEY'":"'$__secret_value'"}' 2>&1`
   if [[ "$secret_value" =~ .*"httpcode:403".* ]]; then
     # get brand new token and retry
     ...
     get_enterprise_secret_token true vault_token
     secret_value=`curl -k -s -X POST ${VAULT_ADDR}/v1/secret/lists-v4/${org}/${__env_name}/${gitrepo}/$appname -H "X-Vault-Token: ${vault_token}" -H "Content-Type: application/json" --data '{"'$PRIVKEY'":"'$__secret_value'"}' 2>&1`
   fi
 }
