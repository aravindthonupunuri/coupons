...
class CronEventServiceTest extends Specification {
    ...
    def "Test processCronEvent"() {
        ...
        def registryCoupons11 = new RegistryCoupons("1000000", registry1, CouponType.STORE, CouponRedemptionStatus.AVAILABLE, LocalDate.now(), LocalDate.now().plusDays(2), null , null)
        def registryCoupons12 = new RegistryCoupons("1000000", registry1, CouponType.ONLINE, CouponRedemptionStatus.AVAILABLE, LocalDate.now(), LocalDate.now().plusDays(2), null , null)
        ...
        def registryCoupons22 = new RegistryCoupons("3000000", registry2, CouponType.ONLINE, CouponRedemptionStatus.AVAILABLE, LocalDate.now(), LocalDate.now().plusDays(2), null , null)
        ...
        def registryCoupons41 = new RegistryCoupons("4000000", registry4, CouponType.ONLINE, CouponRedemptionStatus.AVAILABLE, LocalDate.now(), LocalDate.now().plusDays(2), null , null)
        ...
        1 * registryCouponsRepository.saveAll(_ as List<RegistryCoupons>) >> { arguments ->
            ...
            assert list.size() == 2
            Flux.just(registryCoupons11, registryCoupons12)
        }
        1 * couponsRepository.deleteByCouponCodeInList(_ as List<String>) >> { arguments ->
            final List<String> list = arguments[0]
            assert list.size() == 2
            Mono.just(2)
        }
